...
define("FILENAME","cache/".USERNAME."-followers.csv");

// Check we can open this file
if (!$handle = fopen(FILENAME, 'a')) {
	die("Cannot open file for writing: ".FILENAME."\n");
} else {
	// File is opened, write the CSV headers
	fwrite($handle, "\"screen_name\",\"name\",\"description\",\"image\",\"location\",\"url\",\"followers\",\"friends\"\n");
}
...
try {
	...
	while ($cursor != "0") {
		...
		foreach ($response->users as $user) {
			$csv = "\"".$user->screen_name."\",\"".$user->name."\",\"".$user->description."\",\"".
				$user->profile_image_url."\",\"".$user->location."\",\"".$user->url."\",\"".$user->followers_count."\",\"".$user->friends_count."\"\n";
			fwrite($handle,$csv);
		}
	}
